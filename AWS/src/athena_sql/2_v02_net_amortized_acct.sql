-- Produces data required for net amortized cost reporting by account
-- Includes 1month, 3m, 11m lookback

CREATE OR REPLACE VIEW "athena_db_name".cur_tbl2_v02_net_amortized_acct AS
SELECT 
...
(
  ...
  FROM
    (
      ...
      ON tbl_accts.line_item_usage_account_id = tbl_cost.line_item_usage_account_id
      -- join dates
      LEFT JOIN 
      (
        SELECT 
