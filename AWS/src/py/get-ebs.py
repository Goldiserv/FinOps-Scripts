...
b3 = boto3.session.Session(
    profile_name=os.getenv("AWS_PROFILE"), region_name=regionName
)
...
def get_ebs_data(sts_client, sub_account, inner_role_name, payer_account_id):
    volumeIds = []
    try:
        # assume cross-account IAM role to connect to sub-account
        if sub_account["Id"] != payer_account_id:
            response = sts_client.assume_role(
                RoleArn="arn:aws:iam::{}:role/{}".format(
                    sub_account["Id"], inner_role_name
                ),
        ...
        print(
            "ERROR Failed to assume cross-account IAM role ACCOUNT ID: {} | ERROR: {}".format(
                sub_account["Id"], e
            )
        )
        appendRow = defaultRow.copy()
        appendRow["Account ID"] = sub_account["Id"]
        appendRow["Account Alias"] = sub_account["Name"]
        appendRow["Note"] = "No access to account"
    ...
    if sub_account["Id"] != payer_account_id:
        ...
        inner_session = boto3
        print(" - Connected to payer-account")

    # loop regions
    tempClient = boto3.client("ec2")
    regions = [
        region["RegionName"] for region in tempClient.describe_regions()["Regions"]
    ]

    for region in regions:
        # inner_session = boto3.Session(profile_name=profile, region_name="ap-southeast-2")
        ec2 = inner_session.client("ec2", region_name=region)
        cloudwatch = inner_session.client("cloudwatch", region_name=region)
        ...
        for r in response:
            ...
            try:
                ...
                vBb = get_cw_metrics_extended(
                    volumeId, cloudwatch, "BurstBalance", ["p99"]
                )
                ...
                try:
                    ...
                    eMsg = "ERROR {} read/write latency nitro: {}".format(
                        volumeId, repr(e)
                    )
                    note_2.append(eMsg)
                    print(eMsg)
                ...
                try:
                    volumeReadWriteBytesPeakOfSum_MiB_perSec = (
                        compressStats([vRb, vWb], [], ["Sum"], True)
                        / periodGlobal
                        / 1024
                        / 1024
                    )
                    ...
                    eMsg = (
                        "ERROR {} volumeReadWriteBytesPeakOfSum_MiB_perSec: {}".format(
                            volumeId, repr(e)
                        )
                    )
                    note_2.append(eMsg)
                ...
                cost_GB_month = (
                    pricingEbsSyd["size_gb_month"][r["VolumeType"]] * r["Size"]
                )
                cost_PIOPs_month = (
                    pricingEbsSyd["piop_month_0_to_3000"][r["VolumeType"]]
                    * piops_0_to_3000
                ...
                cost_est_monthly = (
                    cost_GB_month + cost_PIOPs_month + cost_Throughput_month
                )
                ...
                errMsg = "ERROR: {} | EBS data: {}".format(repr(e), r)
                print(errMsg)
                appendRow = defaultRow.copy()
                appendRow["Account ID"] = sub_account["Id"]
                appendRow["Account Alias"] = sub_account["Name"]
...
def get_ebs_data_all_accts(
    inner_role_name, payer_account_id, skip_accounts=[], only_include_accounts=[]
):
    # get organization details
    org_client = b3.client("organizations", region_name="us-east-2")
    org_accounts = call_boto_pagination(
        func=org_client.list_accounts, key="Accounts", token_name="NextToken"
    ).get("Accounts", [])
    ...
    sts_client = b3.client("sts", region_name="us-east-2")
    ...
    for account in org_accounts:
        ...
        if account["Status"] != "ACTIVE":
            continue
        # # skip payer account
        # if account['Id'] == payer_account_id:
        #    continue
        # skip accounts
        if (account["Id"] in skip_accounts) or (
            only_include_accounts and account["Id"] not in only_include_accounts
        ):
            print("INFO Skipping account ID: {}".format(account["Id"]))
            appendRow = defaultRow.copy()
            appendRow["Account ID"] = account["Id"]
            appendRow["Account Alias"] = account["Name"]
            appendRow["Note"] = "Account skipped"
            results.append(appendRow)
            continue

        print("INFO Working on account ID: {}".format(account["Id"]))

        ebs_volume_data = get_ebs_data(
...
if __name__ == "__main__":
    ...
    get_ebs_data_all_accts(
        ...
        # <-------- please provide AWS Account ID
        payer_account_id=os.getenv("AWS_PAYER_ACCT_ID"),
        # Cross-account IAM Role name
        # <-------- please provide cross-account role name
        inner_role_name="OrganizationAccountAccessRole",
        # List accounts to skip
        skip_accounts=[],
        # Only runs theses accounts, ignored if blank (i.e. nil entries) , '886495735684'
        only_include_accounts=[],
    )
