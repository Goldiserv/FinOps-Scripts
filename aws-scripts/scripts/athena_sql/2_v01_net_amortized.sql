-- Sums CUR cost columns to match net amortized cost from Cost Explorer 

CREATE OR REPLACE VIEW "athena_db_name".cur_tbl2_v01_net_amortized AS
SELECT * FROM
(
  SELECT 
    year
...
-- Placeholder for columns that exist in CUR
-- , sum(line_item_net_unblended_cost) line_item_net_unblended_cost
-- , sum(line_item_unblended_cost) line_item_unblended_cost
